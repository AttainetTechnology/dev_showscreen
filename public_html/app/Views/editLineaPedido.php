...
<style>
    .select2-container {
        width: 100% !important;
        /* Asegura que el ancho coincida con el input */
    }

    .select2-container .select2-selection--single {
        height: calc(2.25rem + 2px);
        /* Ajustar altura para que coincida con Bootstrap */
    ...
    .select2-dropdown {
        z-index: 1055;
        /* Asegura que el dropdown se muestre sobre el modal */
    }
...
<div class="content-body">
    <form class="editLineaForm" action="<?= base_url('pedidos/updateLineaPedido/' . $linea_pedido['id_lineapedido']) ?>"
        method="post" data-linea-id="<?= $linea_pedido['id_lineapedido'] ?>">
        ...
        <div class="form-group">
            ...
            <input type="number" name="n_piezas" class="form-control" value="<?= esc($linea_pedido['n_piezas']) ?>"
                required>
        </div>
        <div class="form-group">
            <label for="precio_venta">Precio Venta:</label>
            <input type="number" step="0.01" name="precio_venta" class="form-control"
                value="<?= esc($linea_pedido['precio_venta']) ?>" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="date" name="fecha_entrada" class="form-control"
                value="<?= esc($linea_pedido['fecha_entrada']) ?>">
        </div>
        <div class="form-group">
    ...
    $(document).ready(function () {
        function initializeSelect2() {
            $('#id_producto').select2({
                ...
                width: '100%', // Asegurar que coincida con el ancho del input
                dropdownParent: $('.content-body') // Asegurar que el dropdown aparezca dentro del modal
            });
        }
        ...
        // Re-inicializa Select2 cada vez que se abre el modal
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        location.reload(); // Refresca la página para mostrar cambios
                    } else {
