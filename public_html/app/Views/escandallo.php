...
<div class="botonSeparados">
    <a href="<?= base_url('/lista_produccion/todoslospartes') ?>" class="boton volverButton">
        Volver
...
<?php if (isset($relaciones) && !empty($relaciones)): ?>
    <script>
        ...
        const columnDefs = [
            {
                ...
                cellRenderer: function (params) {
                    if (params.data.tiene_relaciones) { 
                        return `<button onclick="redirigirEscandallo('${params.data.id_proceso_pedido}')" class="btn botonTabla accederButtonTabla">
                    Detalles
        ...
        document.getElementById('volverButton').addEventListener('click', function () {
            window.location.href = '<?= base_url('lista_produccion/todoslospartes') ?>';
        });
    </script>
<?php else: ?>
    <p> </p>
