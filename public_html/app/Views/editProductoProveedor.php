...
<br>
<br>
<form action="<?= base_url('productos_necesidad/update/' . $producto['id_producto']) ?>" method="post"
    enctype="multipart/form-data" class="fromEditProductProveed">
    <div class="mb-3">
...
<script>
    $(document).ready(function () {
        ...
        // Actualiza la imagen seleccionada cuando el modal se cierre
        function selectImage(imageUrl) {
            $('#imagenSeleccionada').val(imageUrl); // Guarda la URL de la imagen en el campo oculto
            $('#imagenSeleccionadaPreview').attr('src', imageUrl).show(); // Muestra la vista previa
            $('#galleryModal').modal('hide'); // Cierra el modal
        }
    });
