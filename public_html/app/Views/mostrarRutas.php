...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const columnDefs = [
            {
                headerName: "Acciones",
                cellRenderer: params => `
                    <button class="btn botonTabla btnEditarTabla" onclick="editRuta(${params.data.id_ruta})">Editar
                    ...
                    <button class="btn botonTabla btnEliminarTabla" onclick="deleteRuta(${params.data.id_ruta})">Eliminar
            ...
            { headerName: "Fecha", field: "fecha_ruta", sortable: true, filter: 'agDateColumnFilter' },
            {
                ...
                sortable: true,
                filter: true,
                valueFormatter: params => estadoMap[params.value] || " ",
                cellRenderer: (params) => {
                    const estado = params.value;
                    let iconHtml = '';

                    if (estado === "1") {
                        // Si el estado es "1", mostrar el ícono
                        iconHtml = `
                <span class="estado-icono" onclick="cambiarEstadoRuta(${params.data.id_ruta}, '2')">
              ...
              return `
            <span>${estadoMap[estado]}</span>
            ${iconHtml}
        `;
                }
            },
            { headerName: "Cliente", field: "nombre_cliente", sortable: true, filter: true },
            { headerName: "Población", field: "nombre_poblacion", sortable: true, filter: true },
            ...
            { 
                ...
                sortable: true, 
                filter: true,
                valueFormatter: params => recogidaEntregaMap[params.value] || "Desconocido"
            },
            { headerName: "Transportista", field: "nombre_transportista", sortable: true, filter: true },
            {
                headerName: "Pedido",
                field: "id_pedido",
                cellRenderer: params => {
        ...
        // Función para cargar datos de rutas
        function fetchRutasData(gridApi) {
            fetch('<?= base_url("rutas/getRutas") ?>', {
                method: 'POST',
        ...
        document.getElementById('add-ruta-btn').addEventListener('click', function () {
            window.location.href = '<?= base_url("rutas/add_ruta") ?>';
        });
        ...
        window.editRuta = function (id) {
            window.location.href = `<?= base_url("editar_rutas") ?>/${id}`;
        };

        window.deleteRuta = function (id) {
            if (confirm('¿Estás seguro de eliminar esta ruta?')) {
                fetch(`<?= base_url("rutas/deleteRuta") ?>/${id}`, { method: 'DELETE' })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            gridOptions.api.applyTransaction({ remove: [{ id_ruta: id }] });
                        } else {
    ...
    function cambiarEstadoRuta(idRuta, nuevoEstado) {
        fetch(`<?= base_url("rutas/cambiarEstado") ?>/${idRuta}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
